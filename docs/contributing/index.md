...
> [!IMPORTANT]
> Unofficial fan project, **not affiliated with Nintendo**. **Never commit ROMs, game assets, or
> extracted data.** PRs that add such material are rejected — and the [Copyright Gate](copyright-gate.md)
> will flag them automatically.
...
2. **[Coding Standards](coding-standards.md)** — ≤200 lines/file, arrow functions, exports at end,
   one-thing-per-file, deep folders. **Mechanically enforced** (ESLint + hooks).
...
- Branch from `master`; keep PRs small and focused; fill out the PR template.
- Update docs in [`docs/`](../README.md) when behavior changes — they're the source of truth and sync to the Wiki.
- Security-sensitive reports: follow [Security](../project/security.md), don't open a public issue.

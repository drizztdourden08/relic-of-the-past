...
- **Nintendo references:** trademarked terms such as Zelda, Hyrule, Ganon, Triforce, Master Sword, and Nintendo in added text. Paths that legitimately use these names are skipped, including `docs/`, `*.md`, `shared/game/data/records/`, `shared/input/data/`, `shared/credits.ts`, `LICENSE`, and the gate itself. The lists live in `scripts/copyright-gate/patterns.mjs`.

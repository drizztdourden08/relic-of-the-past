...
## Placement guide for "I'm adding X, where does it go?"
...
| Talks to the running game / WASM | `apps/web/src/lib/game/` | New C function → `add-wasm-function` (C + bridge; `EMSCRIPTEN_KEEPALIVE` auto-exports, so there is no export list). |

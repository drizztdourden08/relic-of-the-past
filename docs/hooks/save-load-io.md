...
|----------|-----------|--------|
| `WasmInitHeadless` | `int(void)` | Loads `zelda3_assets.dat` and initializes the core **without SDL** (no render/audio). For Node scripts that only build grids; call after `Module` loads with `noInitialRun`. Returns `1`. |
...
> Save-*states* (machine snapshots, F1–F8) are distinct from *SRAM* (the in-game battery save). See

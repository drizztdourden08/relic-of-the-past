...
Multi-byte values are **little-endian** (`lo, hi`). Buffers are static and reused, so **read/parse
immediately** — don't hold the pointer across frames. Each function page below gives the exact
byte-by-byte layout.
...
2. **JS wrapper** (`ccall` site) in `apps/desktop/src/lib/game/`.
...
> list. (Historically a hand-maintained list lived in both `build.bat` and `Makefile` and drifted; it
> was redundant with `KEEPALIVE` and has been removed.)
>
...
> `--dump-*`), not the interactive renderer — so they have **no renderer `ccall` by design**. They are
...
- **Source** — the `core/…` file + the C signature (verified against the source, not paraphrased).
...
- Pointers are into `mod.HEAPU8`; `void` exports take effect immediately on the next frame.

<!-- @layer docs @kind doc -->
# Game Hooks Reference for the C↔JS boundary
...
immediately instead of holding the pointer across frames. Each function page below gives the exact
byte-by-byte layout.
...
|----------|------:|------|--------|
| State Queries for Inventory & Progress | 9 | [→](state-queries-inventory.md) | `state_queries.c`, `ui_state.c` |
| State Queries for Rooms & Collision | 15 | [→](state-queries-rooms.md) | `state_queries.c`, `state_queries_grids.c`, `state_queries_rooms.c`, `state_queries_room_exits.c` |
| State Queries for Navigation Tables | 6 | [→](state-queries-navigation.md) | `state_queries_tables.c` |
| State Queries for Sprites | 4 | [→](state-queries-sprites.md) | `state_queries_sprites.c`, `state_queries_grids.c` |
...
- **Source:** the `core/...` file plus the C signature, verified against the source, not paraphrased.

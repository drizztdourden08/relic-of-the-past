...
// floor reads as something worth walking to, not as scenery. The hold-up
...
// over it reads as two conflicting light sources, not one glint.
//
...
// per draw instead of read back from the sprite, because the palette row a draw settled
...
// actually loaded it, not with a colour assumed at build time.
static uint8 LightestIndex(uint8 row) {
  ...
  for (int i = 1; i < 16; i++) {
    uint16 color = pal[i];
    // 5 bits per channel, blue high, so brightness is close enough as their plain sum.

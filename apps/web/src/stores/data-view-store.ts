 ...
 * rows, the selected record and any unsaved editor draft. In-memory only: it
 * survives switching collections but not an app restart. Keyed like durable
 * snapshots (`ViewKey`) so the two line up under `useViewState`.
 *
 * It also carries the one REQUEST any surface can make of that view: open this
 * recommendation. A widget cannot reach the page state (`useState` in the app
 * shell), so the shell registers how to bring the inspector to the front, a
 * caller hands over what to show, and the inspector consumes it on arrival.
 * Same shape as the search palette's `registerSettings` / `pendingAnchor` pair.
 *
 * `pendingRecord` is the same handoff for a PLAIN record: a widget's "Edit"
 * button, or a reference it renders, jumps to one exact record.
 */
...
interface DataViewStoreState {
   ...
   * anywhere. Brings the inspector to the front (when the shell has registered
   * how) and leaves it as `pendingRecommendation` for the view to pick up.
   */
...
/**
 * Kept beside the store, not in it: a capability the shell lends, not rendered
 * state, and holding it in the store would re-render every subscriber on re-register.
 */

 ...
 * placement. It is built under the profile's own SEED, so a random shop scope
 * opens the shelves this seed will open. A pool that cannot be built lists
 * nothing; the accounting beside it carries the reason. Sprites join the rows only
...
const usePoolListing = (
  snapshot: RandomizerOptionsSnapshot, romFile: string | null, seed = '',
): readonly PoolListingGroup[] => {
  ...
  return useMemo(() => {
    try {
      ...
      const fillWorld = buildFillWorld(fillOptionsFromSnapshot(snapshot, deliverableSets(), {}, seed));
  ...
  }, [snapshot, seed, spritesAvailable, spriteRevision]);
};

...
/**
 * The singleton core holding the Emscripten module reference and
...
const subscribeGameState = (fn: GameStateListener): () => void => {
  ...
  // the status already became 'running'. For example useAutoTest/useSimRun call
...
// Re-export the per-concern bridge facades; the external API is unchanged.
...
export { wasmGetTitleFrame, wasmGetTitleHidden, wasmGetTitlePalette, wasmGetTitlePoly, wasmSetTitleHidden } from './bridge/title';
...
export type { SpriteCombatInfo, CombatTables } from './bridge/combat-tables';

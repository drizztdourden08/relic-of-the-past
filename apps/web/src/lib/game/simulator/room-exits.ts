...
/**
 * Widest a room-to-room crossing can be and still be a doorway. A supertile's outer
 * padding ring is open all the way round, so a flood that reaches it reports a
 * crossing spanning most of an edge (the first castle's basement "scrolled" east
 * into the desert dungeon over 54 tiles). Real doorways measure 2 to 6.
 */
...
/**
 * Drop duplicate exits: same destination AND same way in. Keying on destination
 * alone is wrong: one side of a screen can carry several crossings landing in
 * places that do not connect (the sanctuary grounds' ledge is only reachable by
 * the second of two east crossings, which that dedupe threw away).
 */
...
const detectRoom = (roomId: number, items: TileReq[], entryTile?: GridPos, src?: ScreenRecord, fromKey = ''): DetectedScreen | null => {
  ...
  // Exit-door trigger spots (attr 0x8E): a border touch or door record there means
  // "walk OUT of the dungeon" (the castle's front doors sit right above the basement
  // jail rooms). Memoized per wall spot. The throne room's push-wall (kDoorType_ThroneRoom,
  // native 0x14) counts as blocked until the follower is in tow: its notch tiles flood,
  // but the wall only opens for her. Warp-room doors (native 0x46) teleport instead of
  // scrolling, so their wall spots are blocked for border/edge purposes too.
  ...
  for (const t of run.result.transitions) {
    ...
    // Exit door -> the OVERWORLD screen its entrance sits on (the entrance table knows
    // the area; the room->exit-screen map has gaps). A cached-entrance room leaves by
    // the door it entered through: picking the first table row with this entrance id
    // sent BOTH huts sharing id 101 out at lw-11. See usesCachedEntrance.
    const ow = cached
  ...
  // An exit door's walkable notch also touches the room border, so the landing
  ...
  for (const conn of run.connections) {
    ...
    // Two STANDALONE interiors (rooms the game can leave straight to the overworld:
    // cached entrance or a kExitDataRooms row) never scroll into one another.
    // `ROOM_EDGE_ADJ` proposes roomId±16 regardless, which is how the bomb hut (0x10a)
    // "scrolled north" into the fairy cave (0xfa). Scrolling INWARD is still fine.
  ...
  // Every room the exit table lists HAS a way back outside, flood or not. The loop
  // above only emits that exit when the flood touched the entrance's tile, and a
  // small cave's spawn record can sit outside its walkable floor (room 0xe2: spawn
  // row 59, floor ends row 55), which would strand the run. Anchor it on the
  // outdoor side of the same door.
  if (!doorSpots.size) {

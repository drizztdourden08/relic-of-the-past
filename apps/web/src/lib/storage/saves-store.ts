...
import { stampStateBuffer } from '@app/lib/game/state-stamp';
...
const writeState = async (p: string, slot: number, data: ArrayBuffer): Promise<void> =>
  saves.writeState(files(), p, slot, await stampedU8(data));
...
const createNormalSave = async (p: string, name: string, data: ArrayBuffer, screenshot?: ArrayBuffer): Promise<NormalSaveInfo> =>
  saves.createNormalSave(files(), p, name, await stampedU8(data), optU8(screenshot));
...
const overwriteNormalSave = async (p: string, id: string, data: ArrayBuffer, screenshot?: ArrayBuffer): Promise<NormalSaveInfo | null> =>
  saves.overwriteNormalSave(files(), p, id, await stampedU8(data), optU8(screenshot));
...
const createAutoSave = async (p: string, trigger: 'timer' | 'quit', data: ArrayBuffer, screenshot?: ArrayBuffer): Promise<AutoSaveInfo> =>
  saves.createAutoSave(files(), p, trigger, await stampedU8(data), optU8(screenshot));

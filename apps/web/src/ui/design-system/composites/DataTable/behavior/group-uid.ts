...
/**
 * A group node's `key` is only the stringified value, so two levels — or two
 * branches of the same level — can share one. Collapse state is stored per
 * node, so it needs an identity that includes the ancestry, not just the value.
 */
...
/** Every group node in the tree, depth-first — the seed for "expanded by default". */

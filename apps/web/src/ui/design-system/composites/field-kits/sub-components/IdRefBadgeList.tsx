...
/**
 * An `array` of `idRef` elements, read one chip per entry — the array kit's
 * counterpart to the single-value `idRef` cell. `summarizeList` flattens
 * everything else to one line of raw values, but a list of references is a
 * list of records, and each one deserves its own resolved name, its own
 * `data-id-ref`/`data-target-kind` handoff, and the same `"Name (id)"` format
 * `formatIdRefDisplay` already gives the single-value case — reused here
 * per entry rather than re-derived.
 *
 ...
 * more of the same rather than a third visual convention.
 */

...
/**
 * Disabled-state cover for any setting-gated surface: a Vanilla-Safe-locked control, a widget
 * whose master toggle is off, a binding list for a disabled feature. Unlike developer mode,
 * which hides its surfaces outright, this keeps the content visible under a translucent scrim,
 * states why it's disabled, and links back to the setting that would re-enable it, so nothing
 * the player relies on silently disappears.
 */
...
// the shared lookup instead of a second hardcoded copy of the same string.
...
const DisabledOverlay = (props: DisabledOverlayProps) => {
  ...
  return (
    <Box className={rootClassName}>
       ...
       *  subtree from focus, tab order, and the a11y tree, so the scrim's own message is what
       *  a screen reader announces instead. */}

 ...
 * handful of fields out of a group without asking for the rest. The widget wants
 * this for a wide collection (e.g. `connection`), where every field does not fit
 * a floating panel a few hundred pixels wide.
 *
 * Omitting the list means "no filter at all", so every group `layoutGroups`
 * produced comes through unchanged.
 */

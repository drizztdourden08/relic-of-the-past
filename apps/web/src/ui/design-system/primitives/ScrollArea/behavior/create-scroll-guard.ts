 ...
 * never reaches the caller's `onScroll` — the one thing that keeps two
 * `ScrollArea`s wired to mirror each other from ping-ponging forever.
 *
 ...
 * every event until the node actually reports having reached it — including
 * that final, arrival event — after which it gets out of the way again.
 */
const createScrollGuard = () => {
   ...
   * node actually settled on (post-clamping), so the target can never be one

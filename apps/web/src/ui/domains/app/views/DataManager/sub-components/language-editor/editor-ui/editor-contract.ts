 ...
 * markers at a line's end, the chips, the legend. The document model — how a
 * token stream becomes a stack of lines and back, what a line remembers, the
 * atom's attribute contract, and which extensions make up the schema — belongs
 * to `editor/` and is owned there. Routing every reference through this file
 * means a rename on that side is a one-line fix here rather than a sweep across
 * a dozen components.
 */

...
import './PlayerSpriteManager.css';

interface PlayerSpriteManagerProps {
  onDeleteConfirm: (title: string, message: string, onConfirm: () => void) => void;
}

const stem = (name: string): string => name.replace(/\.zspr$/i, '');

const PlayerSpriteManager = (props: PlayerSpriteManagerProps) => {
  ...
  const handleDelete = useCallback((e: React.MouseEvent, name: string) => {
    e.stopPropagation();
    onDeleteConfirm('Delete Player Sprite', `Delete "${stem(name)}"? This cannot be undone.`, async () => {
  ...
  const list = (
    <>
      ...
      <Box className="data-list">
        {sprites.length === 0 && <EmptyState message="No player sprites imported yet" />}
        {sprites.map((s) => (
          <ListItemRow
            key={s.name}
            icon={s.preview ? <Image className="player-sprite-row__thumb" src={s.preview} alt={s.name} draggable={false} /> : '🧝'}
            name={stem(s.name)}
  ...
  const detail = !selectedEntry ? (
    <Text>Select a player sprite to preview</Text>
  ) : (
    <Box className="player-sprite-detail">
      {selectedEntry.preview
        ? <Image className="player-sprite-detail__preview" src={selectedEntry.preview} alt={selectedEntry.name} draggable={false} />
        : <Text>No preview available</Text>}
      <Text className="player-sprite-detail__name">{stem(selectedEntry.name)}</Text>
    </Box>
  );
...
export { PlayerSpriteManager };
export type { PlayerSpriteManagerProps };

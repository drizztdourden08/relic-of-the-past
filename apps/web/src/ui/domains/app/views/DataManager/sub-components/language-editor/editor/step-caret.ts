 ...
 * equivalent DOM positions, since the end of one span is also the start of the
 * next. Native motion therefore stopped twice per character and the caret
 * visibly jittered backwards between presses. An inserted token is the same
 * problem from the other side: it is one character to the author, and the caret
 * has no business landing anywhere inside it.
 *
 ...
 * one the browser cannot even draw a caret at. The step passes over it and
...
/**
 * A non-empty selection collapses to its own edge instead of stepping past it.

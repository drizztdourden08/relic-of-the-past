...
/** Floating, draggable button over the game view: packages whatever save state was made most
 *  recently (quick, normal, or auto) into a debug report and hands the id to the bug-report
 *  dialog. Self-gates on GameSettings.allowDebugLogging so callers can mount it unconditionally. */
const DebugReportFloatingButton = ({ profileId, gameRunning, onReportPackaged }: DebugReportFloatingButtonProps) => {
  ...
  const { status, sendReport } = useDebugReportCapture(profileId);
  ...
  return (
    <Button
      ...
      onClick={handleClick}
      title={status === 'error' ? 'Could not package the report - try again' : 'Send a debug report for the current save'}
    >

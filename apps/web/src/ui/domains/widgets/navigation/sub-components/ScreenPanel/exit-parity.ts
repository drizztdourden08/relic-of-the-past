 ...
 * has no exit. An edge with no exit is the signature of a screen the run will
 * never leave that way, which used to be invisible in the widget.
 *
...
/**
 * A screen number as a reader-friendly label, using the screen's real name when known.
 * The palace index is REQUIRED to name an indoor room: room numbers collide across
 * palaces and caves (room 0x80 is both a castle jail cell and a graveyard cave).
 */

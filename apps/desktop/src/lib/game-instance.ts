...
/** Tell the WASM core to replace an item when a chest is opened in a specific room. */
export function setItemOverride(roomId: number, originalItem: number, newItem: number): void {
  ...
  currentModule.ccall('WasmSetItemOverride', null, ['number', 'number', 'number'], [roomId, originalItem, newItem]);
  log.app(`[Randomizer] Override set: room ${roomId}, 0x${originalItem.toString(16)} → 0x${newItem.toString(16)}`);
}

/** Clear all item overrides in the WASM core. */
export function clearItemOverrides(): void {
  if (!currentModule) return;
  currentModule.ccall('WasmClearItemOverrides', null, [], []);

 ...
 *
 * Pack and file names come from the renderer, so EVERY name that becomes a path segment
 * is checked for traversal before it is joined; that check is why this is one module.
 * The manifest format comes from shared/storage/msu-edit, so the Node-fs handlers and
 * the FileStore-backed renderer cannot drift on what a `pack.json` is.
 */

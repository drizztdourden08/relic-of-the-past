...
// The invariant Vanilla Safe guarantees: whatever was requested, turning it on
// leaves nothing enabled that diverges from stock cartridge behavior. Asked of
// the registry, not a hand-maintained list, so new features are covered.

describe('Vanilla Safe makes resolveGates lock out every parity-affecting feature', () => {
  ...
  it('cascades through a multi-level requires chain, so killing the root kills every descendant', () => {
  ...
  // Both only observe, but both have GameHook_* call sites compiled into
  // vendored sources (haptics across ancilla.c/player.c/sprite.c/overworld.c,
  // developer tools at misc.c's GameHook_ModuleFrameEnd), and touching that
  // code is the line under Vanilla Safe. This used to exempt
  // developerToolsEnabled as "purely host-side"; applied consistently that
  // exempts every observational feature, which is how tracker notifications
  // ended up wrongly exempt too.

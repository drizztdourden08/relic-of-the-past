...
| Wishing-pond model (`shared/randomizer/ap-world/pond/`): the rupee decomposition and its volleys, the three modes' throw/price/prize schedules, the snapshot adapter and its fallbacks (a retired mode included), the worst-case wallet reading of a prize, and the pond's own receipt lines | tests/randomizer/pond-model.keep.test.ts | covered |

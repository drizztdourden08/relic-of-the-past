...
// The recursive walk from schema to choosable tree, and the adapter that hands
// it to a menu. Where the entries end up is asserted in data-table-menus.test.ts.
...
describeDataset('FieldPicker and its recursive walk', () => {
  ...
  it('treats an array as a leaf, so element paths are not addressable', () => {
...
describeDataset('the field tree as menu entries', () => {
  ...
  it('keeps nesting, so a menu can open a grandchild instead of showing a dead row', () => {
  ...
  it('says so, greyed, instead of offering an empty submenu', () => {

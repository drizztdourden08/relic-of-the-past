...
| `state-big-screen.keep.spec.ts` | Multi-screen areas silently collapsing to the one sub-screen the player stands on — a regression that still produces a plausible-looking number. The castle exterior spans four screens, so the total must be 4 × 4096 = 16384. |
| `state-sanctuary-grounds.keep.spec.ts` | The outdoor navigation baseline. Overworld floods take a different path than bounded indoor rooms (walkable screen edges, an entrance to step into), so one baseline cannot cover both. Pins a single-screen total of 4096 and 1762 reachable. |

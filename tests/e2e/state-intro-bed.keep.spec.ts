 ...
 *
 * `test-intro-bed` is the first frame of the game and the only state with
 * `link_player_handler_state == kPlayerState_AsleepInBed`, so it is the only
 * way to check the "Asleep in bed" / "Waking up in bed" chips against real
 * memory. "Asleep" comes from the handler state; `player_sleep_in_bed_state`
 * is a step counter that reads ZERO during sleep. Getting that wrong shows as a
 * chip that never changes, or says "waking" from the first frame.
 *
 * So this spec plays the opening and asserts each beat:
 *
 ...
 *
 * Beat 3 is also the zero baseline for progress chips: the state starts with
 * none, so the ones that appear are the ones the intro sets.
 *
 * Every beat waits on a chip predicate (`awaitState`), never a frame count.
 */
...
/**
 * Tap through the scene until the chips satisfy `want`. The budget is a ceiling
 * on taps: chips are checked FIRST each iteration, so a fast machine leaves
 * early. Only exhausting it is a failure, and the message carries the chips seen.
 */
...
test('test-intro-bed plays the opening and reports each beat', async () => {
  ...
  await withState('test-intro-bed', async (r) => {
    ...
    // wrote it, so a chip appearing here is a true positive instead of noise.

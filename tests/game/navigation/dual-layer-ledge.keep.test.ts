...
// Regression for commit 2de348d9: tagging ledge tiles as traversal replaced
// expandLedgeCross's landing-cell return with `return []`, so a ledge drop drew
// the arrow but never flooded the layer-1 area (e.g. room 0x72 East Corridor).
...
describe('DualLayerStrategy enqueues the layer-1 landing of a ledge drop', () => {
  it('returns the landing cell on layer 1 when Link drops south off a 2-wide ledge', () => {
    ...
    // Layer 1 is open floor, the lower area Link falls into.
  ...
  // Regression for room 0x72's west walkway: a NORTH jump enters a 1-deep
  // trigger line. The old check required the whole 2x2 body on ledge tiles,
  // which straddles the approach floor, so 1-deep fans never fired.
  ...
  it('refuses a 1-wide ledge, because the 2-wide leading edge cannot fit', () => {
    const layer0 = fill({ type: 'free' });
    ...
    // Both possible approach bodies straddle the 1-wide column, so no jump either way.

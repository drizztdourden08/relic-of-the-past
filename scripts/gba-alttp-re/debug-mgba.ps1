...
Write-Host 'Starting mGBA. In mGBA, choose Tools > Start GDB server... and use port 2345.'
Write-Host 'Windows may request firewall approval the first time. Only allow networks you trust.'
Start-Process -FilePath $executable -ArgumentList @((Resolve-Path -LiteralPath $Rom).Path)

...
const mediaRule = (changes) =>
  changes.files
    ...
    .map((file) => ({
      ...
      severity: 'block',
      file,
      hint: 'media file (image/audio/video/font/rom). Add [allow-copyright] when this is intentional',
    }));
    ...
    .flatMap(([file, lines]) =>
      lines
        ...
        .map((line) => ({
          ...
          hint: 'Nintendo reference in added line. Verify it, then [allow-copyright] or reword',
        })));

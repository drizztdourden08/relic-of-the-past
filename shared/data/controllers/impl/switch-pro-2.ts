...
class SwitchPro2Controller extends BaseController {
  ...
  async init(ctx: ControllerContext): Promise<void> {
    ...
    try {
      ...
      // Send init commands
      await device.transferOut(endpointOut.endpointNumber, new Uint8Array(INIT_COMMAND_0x03));
      await ctx.delay(50);
      await device.transferOut(endpointOut.endpointNumber, new Uint8Array(ENABLE_HAPTICS));
      await ctx.delay(50);

      ctx.log('SPC2: USB bulk init complete — haptics enabled');

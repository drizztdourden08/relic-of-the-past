 ...
 *
 * The ambient channel is one byte wide, so all 63 ids are writable in principle — but only a
 ...
 * 2. The only other writer of `sound_effect_ambient` is `overworld_music[...] >> 4` — a 4-bit
 *    nibble, so that route cannot produce anything above 0x0F.
 ...
 *
 * The roles are what the ids do rather than what they are called: a `bed` starts a loop, the

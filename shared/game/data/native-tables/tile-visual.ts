 ...
 * C:523 rather than a real `default:`.
 */
...
const TERRAIN_VISUAL_GROUPS: readonly AttrGroup<TileVisual>[] = [
  // C:262 TileBehavior_NothingOW — inert either way, but the outdoor branch is what marks
  // it plain walkable ground, so indoors it reads as interior floor.
  {
    attrs: [
  ...
  // C:270 / C:276 / C:283 — three cases that are terrain outdoors and solid indoors. The
  // attribute alone cannot say what the indoor graphic is, so 'wall' records only that it
  // blocks, which is all the engine tells us.
  ...
  // C:300 / C:322 — the two overlay-mask cases; both feed the same pair of layer-change
  // flags (player.c:4206-4218), one for each level, so both read as steps between layers.
  ...
  { attrs: [0x0e, 0x0f], value: 'ice' }, // C:307 GanonIce / C:310 PalaceIce — same look

  // C:313 / C:317 — 'Slope' is the cliff face proper, 'SlopeOuter' the outer corner
  ...
  // C:325 / C:330 / C:339 / C:366 — four staircase cases, one staircase graphic.
  ...
  { attrs: [0x20, ...range(0xb0, 0xbd)], value: 'pit' }, // C:335 Pit — one case
  { attrs: [0x27], value: 'hookshot-post' }, // C:342 Hookshottables

  // C:346-C:365 — five ledge cases, one ledge graphic. Direction lives in the behavior.
  { attrs: range(0x28, 0x2f), value: 'ledge' },

  { attrs: [0x40], value: 'thick-grass' }, // C:371 ThickGrass
  { attrs: [0x44], value: 'spikes' }, // C:374 Spike — spikes/cactus, NOT a dig patch
  ...
  // read action keys off (player.c:5582) — hence bush, bush, sign.
  ...
  { attrs: [0x63], value: 'chest' }, // C:434 MinigameChest — separate case, same look

  // C:423 RupeeTile — indoors only; outdoors the byte is plain walkable ground.
  { attrs: [0x60], value: 'ground', interior: 'rupee-floor' },

  { attrs: [0x67], value: 'crystal-peg' }, // C:440 CrystalPeg_Up — a crystal switch peg
  { attrs: range(0x68, 0x6b), value: 'conveyor' }, // C:445-454, four direction cases

  // C:505-522 — outdoor-only cases; indoors the body is skipped, leaving inert floor.
  ...
  // (player.c:5182), so they read as ledges rather than as unknown terrain.

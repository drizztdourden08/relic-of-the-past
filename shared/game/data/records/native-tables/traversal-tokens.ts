 ...
 */
import type { TraversalRequirement } from '@shared/game/navigation/nav-data.types';
import type { ConnectionTag } from '@shared/game/data/taxonomy/connection-tags';
...
const ITEM_TO_TOKEN: Partial<Record<ItemId, TraversalRequirement>> = {
  ...
  // the flood at all, so the wall stayed shut forever. Receiving bombs also did not
  // count as traversal-affecting, so a wall already seen was never re-evaluated
  // once they arrived. The well's own chests hand over the bombs that open its top
  // room, so both halves have to work for that chest to be reachable.
 ...
 * every run has from the start, so it is granted unconditionally and not listed
 * here.
 */
 ...
 * is absent on purpose because it adds no requirement.
 *

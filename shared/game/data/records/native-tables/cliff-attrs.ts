 ...
 * only gathers the byte sets under one roof. It does not change which consumer
 * reads which set, or how; that rewiring is separate follow-up work.
 *
 ...
 * case and 0x18-0x1B as one `SlopeOuter` case, with no face/edge distinction
 * inside either run. This project re-cuts those same four-byte runs into two
 * conceptual buckets:
 ...
 * synthesise ledges (a face extends a ledge run when the player character
 * jumps into it; an edge does not). No single export below uses either full
 * four-byte bucket. Each original call site picked whatever subset it needed,
 * and those subsets disagree with one another. See the consolidation report
 * for the discrepancy table. Nothing here has been unified or "corrected".
 * Every set is preserved byte-for-byte from its original call site.
 */
import type { CliffDir } from '@shared/game/data/types/native-tables';
 ...
 * (see `VERTICAL_CLIFF_DIRS`) and run through `processDiagonalCliffs`, not this one.
 */
 ...
 * direction apiece, so no per-tile inference is needed.
 */
 ...
 * Includes 0x04, which `CLIFF_WALL_OUTDOOR` below does not, because thick
 * grass is walkable outdoors but a wall indoors.
 */
 ...
 * Empty in the current source, because diagonal cliffs are handled entirely
 * by `processDiagonalCliffs` and never seed a south run. It stays a named
 * export instead of being inlined. That is how the original reads, and the
 * emptiness is load-bearing (see the comment at its call site about
 * re-emitted diagonal tiles).
 */
 ...
 * bytes (0x10, 0x18), so it leaves out the rest of the four-byte "cliff-face"
 * bucket described above and the NORTH/WEST face bytes 0x12/0x1a that
 * `CLIFF_WALL_*` carries instead.
 */
 ...
 * `HORIZ_LEDGE_ATTRS`). These are half of `CLIFF_TRIGGERS`, exported
 ...
 * `VERT_LEDGE_ATTRS`). These are the other half of `CLIFF_TRIGGERS`.
 */
 ...
 * covers all four straight triggers, all four diagonal triggers, and the wall
 * set (0x01-0x03, plus 0x12/0x1a face and 0x13/0x1b edge bytes, matching
 * `CLIFF_WALL_OUTDOOR` exactly). It also carries 0x11 and 0x19, the other two
 * "cliff-edge/slope" bytes that no `CLIFF_WALL_*` variant includes, and 0x3d,
 * which appears nowhere else in any of the three original files and whose
 * meaning here is unexplained. This consolidation flags 0x3d without
 * resolving it.
 */

 ...
 *
 * All 256 bytes are covered: the switch ends in `assert(0)` at C:523 instead of a real
 * `default:`, so anything missing here would be a byte the engine treats as impossible.
 */
import type { TileBehavior } from '@shared/game/data/types/tile-attrs-types';
...
const TERRAIN_BEHAVIOR_GROUPS: readonly AttrGroup<TileBehavior>[] = [
  // C:262 TileBehavior_NothingOW is inert. Outdoors it also raises the plain-walkable
  ...
  // C:270 is an unnamed case: solid indoors, inert outdoors.
  { attrs: range(0x6c, 0x6f), value: 'nothing', interior: 'standard-collision' },
  // C:276 is tall grass outdoors, plain collision indoors.
  { attrs: [0x04], value: 'thick-grass', interior: 'standard-collision' },
  // C:283 is an unnamed second deep-water byte (high-nibble flag), solid indoors.
  ...
  // In C:313 / C:317 the low two bits pick one of four diagonal directions within each
  ...
  // In C:335 TileBehavior_Pit, 0x20 and the 0xb0-0xbd run are ONE case.
  ...
  // The two diagonal cases C:358 / C:362 interleave: even bytes north, odd bytes south.
  ...
  // C:366 TileHandlerIndoor_3E is the fifth staircase case. Unlike C:325/C:330 it raises
  // the in-room staircase flag in the high nibble, which selects a different submodule
  // (player.c:4297).
  { attrs: range(0x3d, 0x3f), value: 'indoor-stairs-3e' },

  { attrs: [0x40], value: 'thick-grass' }, // C:371 TileBehavior_ThickGrass
  { attrs: [0x44], value: 'spike' }, // C:374 TileBehavior_Spike does damage, not diggable
  ...
  { attrs: [0x4b], value: 'warp' }, // C:388 TileBehavior_Warp, 0x49 is NOT part of it
  ...
  // C:423 TileBehavior_RupeeTile applies only indoors. Outdoors the byte is plain walkable.
  ...
  // The switch's tail, C:505-522, is three outdoor-only cases whose bodies are wrapped in
  // `if (!is_indoors)`, so indoors they do nothing at all (not even collision).

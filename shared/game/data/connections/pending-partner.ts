 ...
 * `ConnectionRecord.toConnectionId` is required, never optional — see
 * `data/types/connection.ts`). When the destination screen already holds a
 * point aimed back at this one, that partner's real id is used directly. When
 * it does not — a brand-new crossing nothing has registered yet — there is no

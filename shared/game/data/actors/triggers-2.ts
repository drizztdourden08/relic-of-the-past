...
const TRIGGER_ACTORS_2: ActorRecord[] = [
  ...
  // --- Clear-room family widening (bug fix) ---
  ...
  // Ids appended (267 was the prior max) rather than folded into actor-024,
  // because ActorGameId.roomTag is a single number and cannot represent a set.
  {
    // kDungTagroutines[0x29] = RoomTag_NorthWestTrigger — the exact same function
    // as actor-024 (dungeon.c:243), reused for a different set of rooms. Inside
    // RoomTag_QuadrantTrigger the raw tag value (41) is >= 0x29, so this instance
    // reveals a chest rather than lifting a trapdoor (dungeon.c:4400-4420).
    id: 'actor-268',
    gameId: { roomTag: 41 },
    kind: 'trigger',
  ...
  {
    // kDungTagroutines[0x32] = RoomTag_RoomTrigger — the same handler as roomTag 10,
    // but since the raw tag (50) isn't literally 10, it takes the chest-reveal
    // branch instead of the trapdoor branch (dungeon.c:252,4432-4440).
    id: 'actor-271',
    gameId: { roomTag: 50 },
    kind: 'trigger',

/**
 * Region Updater — Writes computed RegionNavData back into region definitions.
 *
...
export interface RegionNavUpdate {
  regionId: string;
  screenIndex: number;
  nav: RegionNavData;
}
...
/**
 * Build RegionNavData for each region that has an inGameIndex with flood data.
 */
export function buildRegionNavUpdates(input: RegionUpdaterInput): RegionNavUpdate[] {
  const { regions, tileStats, floodResults, resolvedEntrances } = input;
  const updates: RegionNavUpdate[] = [];
  ...
  for (const region of regions) {
    ...
    updates.push({ regionId: region.id, screenIndex, nav });
  }
...
/**
 * Write region nav data to a JSON output file.
 */
export function writeRegionNavData(
  updates: RegionNavUpdate[],
  outputPath: string
): void {
  const fs = require('fs');
  ...
  const data = Object.fromEntries(
    updates.map(u => [u.regionId, { screenIndex: u.screenIndex, ...u.nav }])
  );

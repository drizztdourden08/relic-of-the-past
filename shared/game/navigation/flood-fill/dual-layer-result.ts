...
/** Converts dual-layer BFS body-reached state into merged reachability + per-layer grids. */
import type { TilePassability, ReachState, GridPos, TransitionPoint } from '../types';
...
import { bodyTiles, SWAP_STAIR_ATTRS } from './bfs-helpers';
import type { SingleScreenResult } from './single-layer';
import type { BodyReached, TraversedStair, StepBounds } from './dual-layer-steps';
...
interface BuildArgs extends StepBounds {
...
const buildDualLayerResult = (args: BuildArgs): DualLayerResult => {
  const { bodyReached, traversedStairTiles, grids, rawAttrs, tileContext, transitions, minR, maxR, minC, maxC } = args;

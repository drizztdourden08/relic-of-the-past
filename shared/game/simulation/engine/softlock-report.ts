 ...
 */
import type { CheckDefinition, ScreenDefinition } from '../../types';
...
const buildSoftlockReport = (state: EngineState, deps: ReportDeps = {}): SoftlockReport => {
  ...
  const screens = deps.screens ?? ALL_SCREENS;

  const completed = [...state.completedChecks];
  const blocked = checks
    .filter(c => !state.completedChecks.has(c.name) && !state.reachedScreens.has(c.screen))
    .map(c => ({ checkId: c.id, missing: gatesInto(adjacency, c.screen) }));

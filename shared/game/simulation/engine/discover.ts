...
/**
 * Turns in-game-discovered interactables on reachable tiles into triggerable
 * targets. Reachability comes from the observation's `reached` grid, the SAME
 * flood the exit detection ran (entrances, blockers, correct start layer).
 * Re-flooding here with a weaker option set once put split-level rooms on the
 * wrong layer. No grid: the gate is skipped and everything counts as reachable.
 */
...
/**
 * Rows below a chest's stored tile where the player stands to open it. A chest
 * is a 2x2 (16px) block anchored at its top-left 8px tile; the open-chest routine
 * only fires facing up from the first walkable row under the footprint, two rows
 * below the anchor.
 */
const CHEST_OPEN_ROW_OFFSET = 2;

/** True when a chest's open-from tile is reachable. The chest spans `col`/`col+1`
 *  and the 16px-wide player can stand below either, so either `(row + 2, col)`
 *  or `(row + 2, col + 1)` reachable means openable. */
...
/**
 * The follower NPC's two scripted interactions (sprite type 0x76). Transcribed
 * like the NPC presence conditions: the states live in hardcoded C handlers no
 * raw read recovers. Touching her in the cell starts the tagalong (which opens
 * the throne-room passage); reaching the sanctuary with her runs the priest
 * scene that completes the rescue.
 */
...
/**
 * Milestones reached by working a sprite, not by opening anything, so nothing
 * in the save records them unless the run does it. Sprite, room and requirement
 * all live in hardcoded C handlers (see FOLLOWER_STEPS). The shelf needs the
 * follower in tow, as the mantle's handler does (`follower_indicator != 1`
 * returns early). The sage needs nothing: his idle handler routes a pendant-less
 * player straight into the errand, unlike his pendant-gated boots check.
 */
const EVENT_STEPS = [
...
/**
 * Unknown-position interactables (remote rooms) fall back to coarse screen-level
 * reachability. A known position is always local to the observed screen
 * (overworld sprites are resolved to their true screen and local tile first, see
 * `getOverworldSprites`), so it is judged against the flood.
 */
...
/**
 * A chest is a solid 2x2 block the player can never stand on; the game opens it
 * only from the tile directly below its footprint, facing up. So a posKnown chest
 * is reachable iff that open-from tile is (not any neighbour); unknown-position
 * and out-of-flood-range chests keep the coarse fallback.
 */
...
/**
 * Whether a discovered sprite is spawned at the current progress: a sprite is a
 * target only when it maps to a check-giving NPC (CHECK_NPC_FLAGS config) AND
 * that NPC's presence condition holds. Reading it is the sim's one sanctioned
 * data read (see presence-condition.ts): the game gates NPC spawns in hardcoded
 * C prep functions, so the conditions are transcribed as data. Non-check
 * sprites and a missing presenceState both fail open.
 */
 ...
 *  The game's physical position stays wherever the save started, so it cannot
 *  answer which screen an outdoor sprite belongs to. */
 ...
 *  would give, without running the combat sweep. */
...
const discoverTargets = (state: EngineState, obs: SimObservation, reached: Reached): SimTarget[] => {
  ...
  // The game only consults Sprite_CheckIfRoomIsClear in a room whose tag gates
  // on it, so the sweep only runs there.
  ...
  // Open shutters with killables still alive: walking deeper slams the doors
  // shut behind the player (the game's trap-door rule), so every target found
  // in this window carries the trap marker.
  ...
  for (const door of inter.doors) {
    ...
    // Any held small/big key qualifies, because key bookkeeping is coarse for now.
  ...
  for (const sprite of inter.sprites) {
    ...
    // resolved to the screen each one sits on. One belonging to a neighbouring
    // screen has no flood here to judge it against; it becomes a target when
    // THAT screen is observed.
    if (sprite.outdoor && owScreen !== null && sprite.roomId !== owScreen) continue;
    // Scripted milestones come first, BEFORE the presence gate: that gate asks
    // "is this sprite's CHECK on offer", which for the sage is his pendant-gated
    // boots, while his errand is available the whole time. Reach uses the door
    // radius: these sprites are furniture worked from a distance, and the
    // mantle's tile sits inside the shelf, tiles away from any standable floor.
    ...
    if (isPullSwitch(sprite.spriteType)) {
      ...
      // A drain switch already thrown wastes a step for no further effect, so
    ...
    // marker in the room's sprite data). Sword-gated.
    if ((sprite.carriesKey || sprite.carriesBigKey) && hasSword) {
      ...
      // The LAST living killable's death satisfies the room's kill tag, so its
  ...
  // Kill-gated shutter doors: clearing the room's enemies opens them, only in
  // rooms whose header TAG is a kill/clear-to-open variant, and only once EVERY
  // gating sprite is confirmed killable (RoomThreat.clearable).

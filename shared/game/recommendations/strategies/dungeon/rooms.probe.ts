...
/**
 * `DungeonRecord.roomScreenIds` against the currently loaded room. An APPEND
 ...
 * room, since `cur_palace_index_x2` names the dungeon outright, so a missing
 ...
 * direct channel for, because `read` only ever receives `(observations, record)`.
...
/** The record's own list, plus the current screen if it is missing. Always a
 *  UNION and never a subset, so an existing member is never dropped here. */
...
const readRoomScreenIds = (observations: ScreenObservations, record: DungeonRecord): Probe<unknown> => {
  ...
  // No resolved screen this pass, so there is nothing provable to append.

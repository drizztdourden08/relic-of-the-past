...
/**
 * Field probe over `ActorRecord.combat` — the health/flags4/per-class damage
 ...
 * simply not seen this time, not proven to have no combat data. So `read`
 * returns `unread()` for that case rather than `known(undefined)`; getting
...
const readCombat = (observations: ScreenObservations, record: ActorRecord): Probe<unknown> => {
  ...
  // Not observed this pass for this sprite type — silence, not "no data".
